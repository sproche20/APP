...
@Component({
  selector: 'app-regtop',
  templateUrl: './regtop.component.html',
  styleUrls: ['./regtop.component.scss'],
})
export class RegtopComponent implements OnInit {
  private path ='top/';
newimage='';
newfile='';
musicas:Musica[]=[]
  ...
  async audio(event:any ) {
   ...
   if (event.target.files && event.target.files[0]) {
    this.newFile=event.target.files[0];
      ...
      let audio_player = document.getElementById("audio_player2") as HTMLAudioElement
